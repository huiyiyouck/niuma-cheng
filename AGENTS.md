...
每次根会话开工，先扫一眼公告板（coordination），再接指挥官的活——工作流的改动不该压在池里等被叫才知道：

1. `STATUS.md` BCR 池 — 有无待评估 / 待落地 / 待回流的 BCR（职责③④）。
2. `STATUS.md` 元信息变更台账 — 有无待我同步 PROJECTS + 订正根索引 + 勾选的在途行（职责②白名单 1/2）。
3. `REQUESTS.md` — 有无误投到参谋长的项（按黑名单转回，不承接）。

发现需响应项：先向指挥官报现状 + 出方案，不擅自落地（参谋不决策）；无则照常接活。
...
| `niuma-cheng-ai` | niuma-cheng 生态内部通用 AI 处理中枢（Agent Hub）— 生态内多项目未来均可调用同一 AI 处理服务，多调用方预留（非对外泛化平台，见 decisions/0002）；首落地 xiaobao news-l1：四维评分（`score`+`reason`）、标签、摘要、翻译、按需工具调用（KB 检索 / 链接读取 / Web 搜索） | 业务·服务方 | Python + FastAPI + LangGraph | v0.1 已交付（news-l1 真实化，2026-07-04 关闭）；已配 git remote，已接入团队工作流 |
| `niuma-cheng-coordination` | 跨项目协调仓库 — 契约 / 需求池 / 状态 / 决策的单一真源，只处理跨项目边界 | 协调·公告板 | 纯 Markdown | 维护中 |
| `niuma-cheng-workboard` | 项目管理工作台（Web 端项目迭代 IDE）— 读写工作台（5 视图：新增项目会话 + 角色映射 + 对话查看 + 迭代时间轴），聚合各项目工作流状态、接入诊断、跨项目需求与阻塞 | 工具 | React 18 + Vite + Tailwind v4 + shadcn（前端，对齐小报）+ 本地 Node 聚合后端 | v0.2 已上线 `workboard.huiyiyou.cloud`（2026-07-07）；v0.3 进行中 |
