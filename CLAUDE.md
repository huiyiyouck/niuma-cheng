...
1. **结构跟着物理走**：一个头衔 = 一个目录 + 一个会话；无座位不发头衔。
...
| ② | 维护公告板 | 对 coordination 仓**字段级白名单直写**：PROJECTS 同步、台账勾选、BCR 回流状态推进、立项登记 |
| ③ | 协调工作流回流 | 真源 baseline 合 main 后，对各下游跑 `sync-downstream.sh` + 逐仓 commit/push + 回填 BCR 回流清单 |
...
- **对公告板（coordination）字段级白名单直写**：只写白名单内字段，白名单外一律不碰。
  - **白名单**：① PROJECTS.md 元信息同步 ② STATUS.md 元信息变更台账两「已同步」列勾选 ③ BCR 池回流清单与回流状态推进 ④ 立项登记（README.md + PROJECTS.md + STATUS.md 三处）
...
- **两个例外**：立项一次性建壳、工作流回流执行权——其他时候不进入子项目做事。
- 默认且必须使用中文交流。
...
agent-workflow 真源 baseline 合 main 后，由参谋长对各下游跑 `sync-downstream.sh` + 逐仓 commit/push + 回填 BCR 回流清单。
...
| `agent-workflow` | 一人公司 AI 组织操作架构 **SOP 真源**（对外品牌：AI 开发团队工作流），工作流入口/baseline/templates/同步脚本的单一真源，只承接 BCR | 框架真源·工作流组 | Markdown + shell scripts | 自我演进中 |
...
- `agent-workflow` 是工作流框架真源（工作流组），下游项目发现问题时写 BCR 入公告板 BCR 池。
